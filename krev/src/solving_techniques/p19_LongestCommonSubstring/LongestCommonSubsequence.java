...
//        System.out.println(longestCommonSubsequencePrintIteratively(text1, text2, dp));
    ...
    public static String longestCommonSubsequencePrintIteratively(String text1, String text2, int[][] dp) {
        int startI = text1.length();
        int startJ = text2.length();
        ...
        while (startI > 0 && startJ > 0) {
            if (dp[startI][startJ] == dp[startI - 1][startJ]) {
                ...
                //i.e. dp[startI][startJ] == dp[startI - 1][startJ - 1] + 1
                //AND both previous cases are skipped => it means that dp[startI][startJ] might be built ONLY on the diagonal cell [startI - 1][startJ - 1]
                sb.append(text1.charAt(startI - 1));
                startI--;
                startJ--;
            }
        }
