...
public class MergeTripletsToFormTargetTriplet {
    ...
    public boolean mergeTriplets(int[][] triplets, int[] target) {
        boolean[] result = new boolean[target.length];  //initially filled by 'false'
        int counter = 0;
        for (int[] t : triplets) {
            //exclude triplets that are definitely not fit since they bring value that is greater than corresponding value in target
            if (t[0] > target[0] || t[1] > target[1] || t[2] > target[2]) {
                continue;
            }

            for (int i = 0; i < 3; i++) {
                if (t[i] == target[i] && result[i] == false) {
                    result[i] = true;
                    counter++;
                }
            }

            //optimization: we do not need to go up to the end of the array if we already know that the answer is 'true'
