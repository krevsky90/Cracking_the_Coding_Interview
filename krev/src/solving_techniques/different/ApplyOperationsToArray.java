...
public class ApplyOperationsToArray {
    public static void main(String[] args) {
        ApplyOperationsToArray obj = new ApplyOperationsToArray();
    ...
    public int[] applyOperations(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] != 0) {
                nums[placeToInsert] = nums[i];  //sometimes it will replace itself (case: [1,2,4,0])
                placeToInsert++;
            }
        }

        //fill the rest part by zeros
        for (int i = placeToInsert; i < nums.length; i++) {
            nums[i] = 0;
        }
